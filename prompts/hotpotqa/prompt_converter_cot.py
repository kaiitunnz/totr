...
def convert_file(input_file, output_file, system_prompt=None, direct=False):
    ...
    for example in examples:
        ...
        answer_parts = full_answer.split("So the answer is:")
        
        if len(answer_parts) > 1:
            thought = answer_parts[0].strip()
            answer = "So the answer is:" + answer_parts[1].strip()
        else:
            thought = full_answer
            answer = full_answer
...
def main():
    parser = argparse.ArgumentParser(description='Convert prompts to llama-instruct format')
    parser.add_argument('--input', '-i', required=True, help='Input file')
    parser.add_argument('--output', '-o', required=True, help='Output file')
    parser.add_argument('--system-prompt', '-s', help='System prompt to add')
    parser.add_argument('--direct', action='store_true', help='Direct mode (no modifications)')
